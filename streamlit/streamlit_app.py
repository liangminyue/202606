...
# ============================================================
# 版本与兼容性说明（重要）：
# 模型(.pkl)由 scikit-learn 1.3.2 序列化，requirements.txt 已锁定
# scikit-learn==1.3.2，确保「运行环境版本 == 模型序列化版本」，可直接在
# Streamlit Cloud 部署。下方为历史兼容垫片：当在更高版本 sklearn 下误加载
# 旧模型时，尽力映射已重命名的 sklearn.ensemble._gb_losses 模块；若当前环境
# 无该模块则静默跳过。真正的修复始终是“运行版本与模型版本一致”。
# ============================================================
import sys
import types
try:
    from sklearn.ensemble import _loss  # noqa: F401
    if 'sklearn.ensemble._gb_losses' not in sys.modules:
        _gb_losses = types.ModuleType('sklearn.ensemble._gb_losses')
        for _attr in dir(_loss):
            if not _attr.startswith('_'):
                setattr(_gb_losses, _attr, getattr(_loss, _attr))
        sys.modules['sklearn.ensemble._gb_losses'] = _gb_losses
except ImportError:
    # 当前环境（sklearn 1.3.2）无 _loss 模块，静默跳过即可
    pass
...
class ModelPredictor:
    ...
    def load_model(self, model_path=None):
        ...
        if model_path is not None:
            ...
            candidates = sorted(WEB_DIR.glob("*.pkl"))
            if not candidates:
                candidates = [None]
